...
func createTable(session *gocql.Session) {
	...
	/*createNamespaceStatement := `create keyspace test with
		replication = {'class':'SimpleStrategy','replication_factor':1};`

	if err := session.Query(createNamespaceStatement).Exec(); err != nil {
		log.Warning("Could not Create Namespace : %v", err)

	}*/

	createTableStatement := `CREATE TABLE IF NOT EXISTS test.sim_inventory (    
...
func insertRows(numberofrows int, session *gocql.Session) {
	...
	defer util.TimeTrack(time.Now(), "insertRows Time")
	imsiN := 132312321
	...
	for i := 0; i < numberofrows; i++ {
		...
		if err := session.Query("INSERT INTO  test.sim_inventory (imsi, msisdn, opc) VALUES (?, ?, ?)",
...
func fetchRows(numberofrows int, session *gocql.Session) {
	...
	defer util.TimeTrack(time.Now(), "fetchRows Time")

	// Fetch multiple rows and run process over them
	iter := session.Query("SELECT imsi, msisdn,opc FROM test.sim_inventory LIMIT ? ", numberofrows).Iter()
...
func dropTable(session *gocql.Session) {
	...
	removeTableStmt := fmt.Sprintf("DROP TABLE IF EXISTS %v", "test.sim_inventory")
...
func alterTable(session *gocql.Session) {
	...
	alterTableStatement := fmt.Sprintf(`ALTER TABLE test.sim_inventory
	ADD  page_no  int ;`)
...
func createIndex(session *gocql.Session) {
	...
	createIndex := fmt.Sprintf(`CREATE INDEX pageindx ON test.sim_inventory(page_no)`)
...
func updateRows(numberofrows int, session *gocql.Session) {
	...
	defer util.TimeTrack(time.Now(), "updateRows Time")

	// Fetch multiple rows and run process over them
	iter := session.Query("SELECT imsi FROM test.sim_inventory LIMIT ? ", numberofrows).Iter()
	...
	for iter.Scan(&imsi) {
		...
		if err := session.Query("UPDATE test.sim_inventory SET page_no = ? where imsi = ?",
...
func fetchRowsChunks(pageno int, session *gocql.Session) {
	...
	defer util.TimeTrack(time.Now(), "fetchRows By Page Time")

	// Fetch multiple rows and run process over them
	iter := session.Query("SELECT imsi, msisdn,opc,page_no FROM test.sim_inventory where page_no = ? ", pageno).Iter()
...
func main() {
	...
	fmt.Println("Cassandra Pagination Test")
	...
	cluster.Keyspace = "test" //this has to be created at start
